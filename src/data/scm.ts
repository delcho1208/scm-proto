import rawCefazolinDashboard from "@/data/cefazolin-dashboard.json";
...
export function getIntegrationRecords(regionId: string, productKey: string): SystemRecord[] {
  if (productKey === rawCefazolinDashboard.product) {
    return rawCefazolinDashboard.integration
      .filter((record) => record.regionId === regionId)
      .map((record) => ({
        system: record.system as SystemKey,
        docNo: record.documentNumber,
        status: record.status as SystemRecord["status"],
