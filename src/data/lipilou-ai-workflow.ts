...
export const lipilouWorkflowSteps: CefazolinWorkflowStep[] = [
  ...
  },
  {
    ...
    status: "완료",
    icon: "account_tree",
    evidence: [`권역 데이터 ${regions.length}/8건 정상`, "현재고·목표재고·재고율 정규화"],
  },
  {
    id: "LIPI-FLOW-03",
    order: 3,
    ...
    status: "완료",
    icon: "factory",
    evidence: alertScenario && "mes_card" in alertScenario
  ...
  },
  {
    ...
    evidence: selectedSolution
      ? [selectedSolution.summary ?? selectedSolution.title, `이관량 ${(selectedSolution.transfer_amount ?? 0).toLocaleString("ko-KR")} BOX`]
      : ["추천안 산출 대기"],
  },
  {
    id: "LIPI-FLOW-07",
    ...
    status: "완료",
    icon: "psychology",
    evidence: selectedSolution ? [selectedSolution.reason, selectedSolution.xai_explanation] : ["XAI 근거 생성 대기"],
  },
  {
    id: "LIPI-FLOW-08",
    order: 8,
  ...
  },
  {
    ...
    icon: "local_shipping",
    evidence: selectedSolution
      ? [`${selectedSolution.from_region} → ${selectedSolution.to_region}`, `${(selectedSolution.transfer_amount ?? 0).toLocaleString("ko-KR")} BOX 이관`]
      : ["승인된 이관 계획 없음"],
  },
  {
    id: "LIPI-FLOW-10",
