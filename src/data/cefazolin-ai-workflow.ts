...
export type WorkflowGroup = "데이터 준비" | "탐지·영향" | "분석·시뮬레이션" | "의사결정" | "승인·실행";
...
const shortageRegionCount = regionalRows.filter((region) => region.stockStatusCode === "danger").length;
const topRiskCauses = [...(national?.policyRisk.causes ?? [])].sort((a, b) => b.score - a.score).slice(0, 3);
...
const supplyRiskCause = national.policyRisk.causes.find((cause) => cause.label.includes("원자재 공급 차질"));
...
export const cefazolinWorkflowSteps: CefazolinWorkflowStep[] = [
  ...
  },
  {
    ...
    shortTitle: "위험 탐지",
    purpose: "통합된 수요·재고·생산·공급 신호를 종합해 수급 이상을 판정하고 의사결정 Case를 생성합니다.",
    icon: "radar",
  ...
  },
  {
    ...
    title: "Case 영향 분석",
    shortTitle: "영향 분석",
    purpose: "탐지된 수급 이상이 권역별 재고와 목표재고 충족률, 영향 범위에 미치는 결과를 사건 단위로 정리합니다.",
    icon: "monitoring",
    ...
    evidence: [
      ...
      ...topRiskCauses.filter((cause) => cause.label !== cefazolinDetectionContext.directSignal).map((cause) => `${cause.label} ${cause.score}/100`),
    ],
    warnings: [],
    nextAction: "동일 Case Snapshot을 기준으로 S1~S3 대응 시나리오를 실행합니다.",
    synthetic: true,
  },
  {
    ...
    dataAsOf: national.policyRisk.asOf,
    evidence: [
      `S1~S3 시나리오 ${cefazolinScenarioComparison.filter((scenario) => scenario.comparisonTarget).length}건`,
      `제약 통과 ${cefazolinScenarioComparison.filter((scenario) => scenario.comparisonTarget && scenario.constraintPassed).length}/${cefazolinScenarioComparison.filter((scenario) => scenario.comparisonTarget).length}건`,
    ],
    warnings: [],
  ...
  {
    id: "FLOW-06-FEASIBILITY",
    order: 6,
    group: "의사결정",
    title: "대응안 실행가능성 검증",
    shortTitle: "실행가능성 검증",
    purpose: "S1~S3 결과의 제약 통과 여부와 미충족 수요·부족기간·서비스율·비용을 검증해 실행 가능한 후보만 남깁니다.",
    icon: "fact_check",
    ruleIds: ["RULE-SCENARIO-CONSTRAINT-001", ...cefazolinScenarioRecommendation.appliedRuleIds],
    dataAsOf: national.policyRisk.asOf,
    evidence: [
      `S1~S3 제약 통과 ${cefazolinScenarioComparison.filter((scenario) => scenario.comparisonTarget && scenario.constraintPassed).length}/${cefazolinScenarioComparison.filter((scenario) => scenario.comparisonTarget).length}건`,
      `재배분 가능 ${formatScmQuantity(rawDashboard.overview.excessTransferableQuantity, "transfer")} · MES 가동률 ${rawDashboard.overview.averageUtilizationPct.toFixed(2)}%`,
      ...(recommendedScenarioId ? [`실행 가능 최우선 후보 ${recommendationTitle}`] : ["실행 가능 권고안 없음"]),
    ],
    ...
    nextAction: "검증을 통과한 후보의 권고 근거와 제외 사유를 확인합니다.",
    synthetic: true,
  },
  {
    id: "FLOW-07-RECOMMENDATION",
    order: 7,
    group: "의사결정",
    title: "최종 권고안 선정·근거",
    shortTitle: "권고안 선정",
    purpose: "실행 가능한 후보 중 우선순위 규칙으로 최종 권고안을 선정하고 모델·시뮬레이션·규칙 근거를 함께 제시합니다.",
    icon: "recommend",
    ruleIds: cefazolinDecisionEvidence.rules,
    dataAsOf: dataAsOfLabel,
    evidence: recommendedScenarioId
      ? [`최종 권고안 ${recommendationTitle}`, ...cefazolinScenarioRecommendation.decisionReasons.slice(0, 2), ...cefazolinDecisionEvidence.simulation.slice(0, 2)]
      : ["유효한 권고안이 없습니다."],
  ...
  },
  {
    ...
    title: "담당자 검토·승인",
    shortTitle: "담당자 승인",
    purpose: "필수 운영 조건을 확인하고 권고안의 승인·보완 여부를 기록합니다.",
    icon: "verified_user",
    ...
    warnings: ["SANDBOX 환경: 실제 전자결재·외부 시스템 승인과 연동되지 않습니다."],
  ...
  },
  {
    ...
    title: "실행지시 준비",
    shortTitle: "실행지시",
    purpose: "승인된 권고안을 ERP·MES·WMS 실행지시 단위로 변환합니다.",
    icon: "play_circle",
    ruleIds: ["RULE-VIRTUAL-EXEC-001"],
    dataAsOf: national.policyRisk.asOf,
    evidence: [
      `실행지시 ${recommendedScenarioId ? "권고안 기준 준비" : "준비 불가"}`,
      "ERP·MES·WMS 전송 상태: SANDBOX",
    ],
    warnings: ["외부 시스템 API·인증·권한은 현재 연결되지 않았습니다."],
    nextAction: "승인 후 시스템별 실행지시를 확인하고 전송 상태를 갱신합니다.",
    synthetic: true,
  },
  {
    id: "FLOW-10-EFFECT",
    ...
    title: "계획 KPI 확인",
    shortTitle: "계획 KPI",
    purpose: "S1 대비 권고 시나리오의 계획 KPI를 확인합니다.",
    icon: "monitoring",
    ruleIds: ["RULE-SIMULATED-EFFECT-001"],
    dataAsOf: national.policyRisk.asOf,
    evidence: recommendedScenario
      ? [
          `S1 대비 ${recommendedScenario.id.replace("_", " ")} 계획 KPI`,
    ...
    nextAction: "실제 운영실적 연계 후 동일 KPI 기준으로 계획 대비 편차를 확인합니다.",
    synthetic: true,
  },
];
